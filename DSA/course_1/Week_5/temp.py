...
class Torrent(Thread):
    ...
    def magnets(self):
        ...
        while True:
            ...
            cprint('Magnet link added {}'.format(
                len(self.tor_file_status)), 'green')
    ...
    def add_torrent(self, link):
        ...
        if torrent_size > self.STORAGE_SIZE:
            ...
            os.replace(tor_path+link, tor_path +
                       'oversized_torrent/' + link.split('/')[-1])
            # try:
    ...
    def check(self):
        ...
        for torrent in ses.get_torrents():
            ...
            cprint('{:.2f}% of {:.2f}GB ( down: {:.1f} mb/s  up: {:.1f} kB/s peers: {:d}) {} \n'.format(
                s.progress * 100, torrent.get_torrent_info().total_size() /
                (1024 * 1024 * 1024),
    ...
    def save_tor_file(self, torrent):
        ...
        if os.path.exists(os.path.join(tor_path, torrent.name() + ".torrent")):
         return True
    ...
    def gui_check(self):
        ...
        for torrent in ses.get_torrents():
            ...
            if torrent in self.torrent_progress.keys():
                ...
                bar.reset()
                bar.n = round(
                    s.progress*torrent.get_torrent_info().total_size() / (1024 * 1024 * 1024), 2)
                bar.last_print_n = round(
                    s.progress*torrent.get_torrent_info().total_size() / (1024 * 1024 * 1024), 2)
                bar.desc = " | ".join([torrent.name()[:40],
                                       self.state_str[s.state],
                                       ])

                d = {'down': str(round(s.download_rate / 1000000, 1))+'mb/s', 'up': str(
                    round(s.upload_rate / 1000, 1))+'kb/s', 'peers': s.num_peers}
                bar.set_postfix(d)
                bar.refresh()
                # bar.pause()
            ...
            if len(self.tor_file_status) != 0:
                ...
                for torrent, status in dict(self.tor_file_status).items():
                    if status:
                        ...
                        self.tor_file_status[torrent] = self.save_tor_file(
                            torrent)
            time.sleep(200)
            if len(ses.get_torrents()) == 0:
              exit(0)
